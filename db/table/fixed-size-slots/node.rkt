...
(library
 (node)
 (export new read write! record record! delete! record-join
 ...
 (define (record-join node slot key-col)
   (define scma (schema node))
   (define blck (block node))
   (define key '())
   (define cons-join (let loop
                       ((cntr 0)
                        (offs (calc-record-offset node slot)))
                       (if (= cntr (scma:nr-of-attributes scma))
                           ()
                           (if (and (null? key)
                               ...
                               (let ((temp-field ((vector-ref decoders (scma:type scma cntr)) blck offs (scma:size scma cntr))))
                                 (set! key temp-field)
                                 (cons temp-field (loop (+ cntr 1) (+ offs (scma:size scma cntr))))
                                 )
                               
                               (cons ((vector-ref decoders (scma:type scma cntr)) 
                                      blck offs (scma:size scma cntr))
                                     (loop (+ cntr 1) (+ offs (scma:size scma cntr))))))))
   (cons key (cons cons-join '()))
   )
