...
(library
 ...
 (export new open close! current current!
         drop! insert! delete! peek peek-join print
 ...
 (define (peek tble)
  ...
  (define (peek-join tble key-col)
   (define bffr (buffer tble))
   (define curr (slot tble))
   (if (null? bffr)
       no-current
       (node:record-join bffr curr key-col)))
 ...
 (define (find-last-occupied-slot node strt)
...
;   (if (null? bffr)
;       no-current
;       (let ((indx (find-last-occupied-slot bffr curr)))
;         (if (= indx -1)
;             (if (
