...
(library
 ...
 (define (for-all-tuples-join table key-col proc)
   (if (not (eq? (tbl:set-current-to-first! table) no-current))
       (let loop
         ((tuple (tbl:peek-join table key-col)))
         (let ((curr (tbl:current table)))
           (if (and (proc tuple curr)
                    (not (eq? (tbl:set-current-to-next! table) no-current)))
               (loop (tbl:peek-join table key-col)))))))
...
(define (for-all-chunks tabl key-col smaller proc)
  ...
  (define dict (make-dictionary smaller))
  (define cap (scma:capacity (tbl:schema tabl)))
  

  (call-with-current-continuation
   (lambda (return)
     
     (let loop-chunk ((block-number 0) (block-index 0) (status (tbl:set-current-to-first! tabl)))
       (cond
           ...
           (else
            ...
            (if (< (+ block-index 1) cap)
                 ...
                 (loop-chunk (+ block-number 1) 0 (if (= (+ block-number 1) (- M 1))
                                                      '()
                                                      (tbl:set-current-to-next! tabl)))
                 ) 
           )
         )
       )
 ...
 (define (select*-from/join dbse t1 t2 a1 a2 smaller)
   (set! output-list '())
   (for-all-chunks t1 a1 smaller
                   (lambda (dict)
