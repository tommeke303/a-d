...
(library
 ...
 (export block-size disk-size
         ...
         cache:requested-reads
         cache:effective-reads
         cache:requested-writes
         cache:effective-writes
         cache-info
         cache:get
         disk-cache
         )
 ...
 (define (cache:requested-reads cdsk)
   (requested-reads? (info cdsk)))
 (define (cache:effective-reads cdsk)
   (effective-reads? (info cdsk)))
 (define (cache:requested-writes cdsk)
   (requested-writes? (info cdsk)))
 (define (cache:effective-writes cdsk)
   (effective-writes? (info cdsk)))

 (define (cache-info cdsk)
   (define inf (info cdsk))
   (list (requested-reads? inf) (effective-reads? inf) (requested-writes? inf) (effective-writes? inf))
   )
 ...
 (define (new name)
   (make-cdisk (cache:new) (disk:new name) (make-info 0 0 0 0)))
 ...
 (define (write-block! blck) 
   ...
   (define inf (info (disk blck)))
   (if (not (valid? blck))
     (error "invalidated block(write-block!)" blck))
   (requested-writes! inf (+ 1 (requested-writes? inf)))
   (locked! blck #f)) ; no write, that's the whole point!
 
 (define (read-block cdsk bptr)
   (define cche (disk-cache cdsk))
   (define cblck (cache:find-block cche bptr))
   (define inf (info cdsk))
   (requested-reads! inf (+ 1 (requested-reads? inf)))
   (if (null? cblck)
     (let*
       ...
       (when (not (null? blck))
         (if (dirty? blck)
           (disk:write-block! (block blck)))
         (effective-writes!  inf (+ 1 (effective-writes? inf)))
         (requested-writes! inf (+ 1 (requested-writes? inf)))
         (invalidate! blck))
       (set! blck (make-block cdsk (disk:read-block (real-disk cdsk) bptr)))
       
       (effective-reads! inf (+ 1 (effective-reads? inf)))
